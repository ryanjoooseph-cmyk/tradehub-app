...
import { TooltipProvider, JobTooltip } from "@/components/ui/tooltip";
...
// Note: Event card colors are now handled via status badges and inline classes
...
export default function CalendarPage() {
  ...
  const pxPerStep = 90; // Increased from 56px for better readability
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="rounded-xl bg-muted/20 p-3">
                  <div className="text-[10px] uppercase tracking-wide text-muted-foreground/70 font-semibold">Jobs</div>
                  <div className="mt-1.5 text-xl font-bold">{events.length}</div>
                </div>
                <div className="rounded-xl bg-success-50 dark:bg-success-950/30 p-3">
                  <div className="text-[10px] uppercase tracking-wide text-success-600/70 dark:text-success-400/70 font-semibold">Active</div>
                  <div className="mt-1.5 text-xl font-bold text-success-600 dark:text-success-400">{inProgressCount}</div>
                </div>
                <div className="rounded-xl bg-warning-50 dark:bg-warning-950/30 p-3">
                  <div className="text-[10px] uppercase tracking-wide text-warning-600/70 dark:text-warning-400/70 font-semibold">Priority</div>
                  <div className="mt-1.5 text-xl font-bold text-warning-600 dark:text-warning-400">{highPriorityCount}</div>
                </div>
                <div className={cx("rounded-xl p-3", conflictCount > 0 ? "bg-danger-50 dark:bg-danger-950/30" : "bg-success-50 dark:bg-success-950/30")}>
                  <div className={cx("text-[10px] uppercase tracking-wide font-semibold", conflictCount > 0 ? "text-danger-600/70 dark:text-danger-400/70" : "text-success-600/70 dark:text-success-400/70")}>Conflicts</div>
                  <div className={cx("mt-1.5 text-xl font-bold", conflictCount > 0 ? "text-danger-600 dark:text-danger-400" : "text-success-600 dark:text-success-400")}>
                    {conflictCount}
                  </div>
                </div>
              </div>
              ...
              {/* Filters */}
              <div>
                <div className="mb-2 flex items-center gap-2 text-[10px] uppercase tracking-wide text-muted-foreground/70 font-semibold">
              ...
              {view === "day" && (
                <div>
                  <div className="mb-2 text-[10px] uppercase tracking-wide text-muted-foreground/70 font-semibold">
                    {isMobile && <span className="text-primary-500">📱 </span>}
                    Select Day
                  </div>
              ...
              {/* Status Legend - Enhanced for PR20 */}
              <div className="rounded-xl border bg-card/30 p-4 shadow-sm">
                <div className="mb-3 text-sm font-bold">Status Legend</div>
                <div className="space-y-2.5">
                  ...
                  <div className="flex items-center gap-3">
                    <Badge status="scheduled" size="sm">Scheduled</Badge>
                    <span className="text-xs text-muted-foreground">Upcoming</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <Badge status="pending" size="sm">Pending</Badge>
                    <span className="text-xs text-muted-foreground">Awaiting start</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <Badge status="completed" size="sm">Complete</Badge>
                    <span className="text-xs text-muted-foreground">Finished</span>
                  </div>
                  <div className="pt-2 border-t border-border/40 mt-2">
                    <div className="flex items-center gap-2 text-xs">
                      <div className="h-2.5 w-2.5 rounded-full bg-rose-500 animate-pulse ring-2 ring-rose-500/20" />
                      <span className="text-muted-foreground font-medium">Urgent priority</span>
                    </div>
                    <div className="flex items-center gap-2 text-xs mt-1.5">
                      <AlertTriangle className="h-3.5 w-3.5 text-danger-500" />
                      <span className="text-muted-foreground font-medium">Scheduling conflict</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className={cx(
                ...
                <div className="relative border-r bg-muted/10" style={{ height: gridHeight }}>
                  {slots.slice(0, -1).map((m) => (
                    <div
                      key={m}
                      className="flex h-[90px] items-start justify-end pr-4 pt-0"
                    >
                      {m % 60 === 0 && (
                  ...
                  return (
                    <div
                      ...
                      className={cx(
                        ...
                        dragOverDay === d && "bg-primary-500/5 border-primary-500 border-dashed border-2"
                      )}
                    ...
                    >
                      ...
                      {/* Events */}
                      {(() => {
                        const dayEvents = visibleEvents.filter((e) => e.day === d);
                        const visibleCount = 3;
                        const hasMore = dayEvents.length > visibleCount;
                        const displayEvents = dayEvents.slice(0, visibleCount);

                        return (
                          <>
                            {displayEvents.map((e) => {
                              const top = ((e.startMin - startDayMin) / stepMin) * pxPerStep + 4;
                              const h = Math.max(120, (e.durMin / stepMin) * pxPerStep - 8);
                              ...
                              return (
                                <TooltipProvider key={e.id}>
                                  <JobTooltip
                                    title={e.title}
                                    location={`${e.site} - ${e.client}`}
                                    time={`${minutesToFull(e.startMin)} – ${minutesToFull(e.startMin + e.durMin)}`}
                                    crew={e.crew}
                                    status={e.status}
                                  >
                                    <div
                                      ...
                                      className="absolute left-1.5 right-1.5 cursor-pointer hover:z-10 transition-transform hover:scale-[1.02]"
                                      style={{ top, height: h }}
                                    >
                                      <div className={cx(
                                        "group h-full rounded-xl border-2 p-4 transition-all hover:shadow-xl overflow-hidden",
                                        e.status === 'in-progress' && "bg-success-500/15 border-success-500 hover:bg-success-500/20",
                                        e.status === 'scheduled' && "bg-info-500/15 border-info-500 hover:bg-info-500/20",
                                        e.status === 'completed' && "bg-neutral-500/15 border-neutral-500 hover:bg-neutral-500/20",
                                        e.priority === 'urgent' && "ring-2 ring-danger-500/30",
                                        isConflict && "ring-2 ring-danger-500 bg-danger-500/15"
                                      )}>
                                          ...
                                          <div className="flex items-center gap-2 min-w-0">
                                            <Badge status={e.status} size="sm">
                                              {e.status === 'in-progress' ? 'In Progress' :
                                               e.status === 'completed' ? 'Complete' : 'Scheduled'}
                                            </Badge>
                                        ...
                                        <div className="text-[14px] font-bold leading-tight mb-3 line-clamp-2">
                                          {e.title}
                                        </div>

                                        {/* Details - Increased font size */}
                                        ...
                                        {h >= 180 && (
                                          <div className="mt-3 flex flex-wrap gap-1.5">
                                            {e.crew.slice(0, 2).map((c, i) => (
                                              <span key={i} className="inline-flex items-center gap-1 rounded-lg bg-background/70 px-2 py-1 text-[11px] font-semibold border border-border/40">
                                                <Users className="h-3 w-3" />
                                                {c}
                                              </span>
                                            ))}
                                            {e.crew.length > 2 && (
                                              <span className="inline-flex items-center rounded-lg bg-background/70 px-2 py-1 text-[11px] font-semibold border border-border/40">
                                                +{e.crew.length - 2}
                                              </span>
                                            )}
                                          </div>
                            ...
                            {hasMore && (
                              <div
                                className="absolute left-1.5 right-1.5 bottom-2 z-20"
                              >
                                <button
                                  className="w-full rounded-lg bg-primary-500/90 hover:bg-primary-600 text-white text-xs font-semibold py-1.5 transition-colors shadow-sm"
                                  onClick={() => {
                                    // Could open a modal showing all events for this day
                                    console.log(`+${dayEvents.length - visibleCount} more events`);
                                  }}
                                >
